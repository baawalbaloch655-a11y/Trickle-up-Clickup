...
const TABS = [
    { id: 'chats', label: 'Recent Chats', icon: MessageSquare, color: 'text-accent-500' },
    { id: 'teams', label: 'All Teams', icon: Hash, color: 'text-blue-500' },
    { id: 'members', label: 'All Members', icon: Users, color: 'text-green-500' },
] as const;
...
export default function ChatLandingPage() {
    ...
    const counts: Record<TabId, number> = {
        ...
        teams: filteredChannels.length,
        members: availableUsers.length,
    };
                    ...
                    <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
                        {TABS.map((tab) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon size={15} className={isActive ? tab.color : 'text-current'} />
                                    {tab.label}
                                    {!isLoading && counts[tab.id] > 0 && (
                                        <span className={clsx(
                            ...
                            {activeTab === 'teams' && (
                                <section>
                                    {filteredChannels.length === 0 ? (
                                        <div className="border border-dashed border-gray-300 rounded-xl p-12 text-center bg-gray-50/50">
                                            <Hash size={32} className="text-gray-300 mx-auto mb-3" />
